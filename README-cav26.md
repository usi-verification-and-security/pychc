...
The artifact allows for replicating the results reported in the experimental evaluation (Section 5 in the submitted paper).

- **Table 1** lists the issues described in
  `pychc/tests/expected_bugs.py`.
  This file includes several test cases, each corresponding to one issue reported in the table. [This section](#table-1) explains in detail how to reproduce the table.

- CHC-COMP25 results: reviewers can replicate the 12 Eldarica failures and the 7 Z3 failures discovered by PyCHC, reported at page 10.
...
PyCHC includes several examples and tutorials (explained in details in [this section](#tutorials-10min)) showing
...
```
This should open a prompt in working directory `~/pychc`
...
```
Output should end with something like `==== 105 passed in 158.64s (0:02:38) =====`
...
### Troubleshooting
If `command not found` is issued for a solver (e.g., `golem`, `eld`, `z3`, `cvc5`, `opensmt`, ...)
...
```
Here, each test should raise error due to a ill-formed formula.
...
```
- Run first 100 benchmarks from the LIA-Lin category, with a timeout of 10s, with Golem.
```
