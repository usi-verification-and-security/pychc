...
if __name__ == "__main__":
    ...
    print(
        f"Loaded CHC form {test_file} has: {len(sys.get_predicates())} predicates, {len(sys.get_clauses())} clauses"
    )
    for clause_id, clause in enumerate(sys.get_clauses()):
        print(f"Clause {clause_id}: {clause.serialize()}")

    print("*" * 20)

    print(f"Running Spacer on {test_file}...")
    spacer = z3.Z3CHCSolver(global_guidance=True)
    spacer.run(test_file)
    spacer_witness = spacer.get_witness()
    print("Spacer found a model!")
    print("Validating the model with CVC5 solver...")
    try:
    ...
    for predicate in sys.get_predicates():
        ...
        print(f"Predicate {pname}({', '.join(str(v) for v in new_vars)}):", end=" ")
