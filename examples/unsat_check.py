...
if __name__ == "__main__":
    ...
    # Create a Golem solver with OpenSMT as validator of SAT answers.
    print("\nSolving with Golem. Validate SAT answers with OpenSMT. UNSAT proofs will be produced in LEGACY (uncheckable) format....")
    ...
    try:
        ...
        print("Error:", e)
    print(f"Golem returned, but proof could not be validated due to legacy format.")

    print("Counterexample produced by Golem:")
    ...
    status = solver.solve(validate=True)
    print(f"Golem returned status: {status}. Proof was validated by Carcara!")
    ...
    print(f"Run Carcara to validate a proof {witness_file} on {sys_file}...")
